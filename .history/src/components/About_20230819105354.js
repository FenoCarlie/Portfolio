...
function About() {
    ...
    const shift = (click) => {
        ...
        const targetSection = clicked.textContent;

        const isDownTransition = targetSection !== "About me";

        // Mise à jour de l'état de la classe de transition
        setTransitionClass(isDownTransition ? 'slideDown' : 'slideUp');
        setShowFirstParagraph(!isDownTransition);

        
    };

    return (
        (<section className={`banner about ${transitionClass}`}>
