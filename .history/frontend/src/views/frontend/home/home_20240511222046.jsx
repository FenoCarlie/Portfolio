...
function Home() {
  ...
  const socialNets = [
    { icon: <FaLinkedin />, link: "www.linkedin.com/in/carlie-feno-516063240" },
    { icon: <FaGithub />, link: "https://github.com/FenoCarlie/" },
    { icon: <FaFacebook />, link: "https://www.facebook.com/aidane.feno" },
    { icon: <FaInstagram />, link: "https://www.instagram.com/carliefeno/" },
  ];
  /***************** */
  ...
  return (
    <div id="home" className="w-full h-full relative">
      <div className="flex justify-between w-full h-full p-4 lg:flex-row xl:flex-row sm:px-10 lg:px-20 xl:px-30">
        ...
        <section className="hidden items-center p-4 lg:flex xl:flex xl:w-[50%] lg:w-[50%] lg:h-full- xl:h-full"></section>
        <section className="h-full items-center flex-col justify-end flex">
          ...
          {socialNets.map((socialNet) => (
            <a href={socialNet.link} key={socialNet.name}>
              {socialNet.icon}
            </a>
          ))}
