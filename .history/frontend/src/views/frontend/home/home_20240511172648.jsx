...
function Home() {
  ...
  const period = 2000;
  const ref = useRef(null); // Add a ref to access the DOM element
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      // Perform operations using the style object
    }
  }, [ref.current]); // Depend on ref.current to re-run effect when it changes
